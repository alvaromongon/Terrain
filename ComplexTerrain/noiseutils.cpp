...
void NoiseMapBuilderSphere::Build()
{
	...
	for (int y = 0; y < m_destHeight; y++) {
		...
		for (int x = 0; x < m_destWidth; x++) {
			...
			// Set values in case there is a river builder
			m_pSourceModule->SetLatLonAndDelta(curLat, curLon, yDelta, xDelta);
...
void RendererImage::Render ()
{
  ...
  RenderPortion(0, 0, width, height);
}

void RendererImage::RenderPortion(int initialWidthPosition, int initialHeightPosition, int width, int height)
{
	if (m_pSourceNoiseMap == NULL
		|| m_pDestImage == NULL
	...
	int sourceWidth = m_pSourceNoiseMap->GetWidth();
	int sourceHeight = m_pSourceNoiseMap->GetHeight();

	// If a background image was provided, make sure it is the same size the
	// source noise map.
	if (m_pBackgroundImage != NULL) {
		if (m_pBackgroundImage->GetWidth() != sourceWidth
			|| m_pBackgroundImage->GetHeight() != sourceHeight) {
			throw noise::ExceptionInvalidParam();
		}
	}

	// If river noise was provided, make sure it is the same size the
	// source noise map.
	if (m_pSourceRiverNoiseMap != NULL) {
		if (m_pSourceRiverNoiseMap->GetWidth() != sourceWidth
			|| m_pSourceRiverNoiseMap->GetHeight() != sourceHeight) {
			throw noise::ExceptionInvalidParam();
		}
	}
	...
	for (int y = 0; y < height; y++) {
		...
		if (m_pBackgroundImage != NULL) {
			pBackground = m_pBackgroundImage->GetConstSlabPtr(yPlusInitial);
		...
		if (m_pSourceRiverNoiseMap != NULL){
			pSourceRiver = m_pSourceRiverNoiseMap->GetConstSlabPtr(yPlusInitial);
			for (int x = 0; x < initialWidthPosition; x++) {
				++pSourceRiver;
			}
		}

		const float* pSource = m_pSourceNoiseMap->GetConstSlabPtr(yPlusInitial);
		for (int x = 0; x < initialWidthPosition; x++) {
			++pSource;
		}
		...
		for (int x = 0; x < width; x++) {
			...
			bool isRiver = false;
			double riverValue = 1.0;
			if (*pSource > 0 && ((m_pSourceRiverNoiseMap != NULL && *pSourceRiver < 0))){
				isRiver = true;
				riverValue += *pSourceRiver;
			}

			// Get the color based on the value at the current point in the noise
			// map.
			Color destColor = m_gradient.GetColor(isRiver ? riverValue : *pSource);
