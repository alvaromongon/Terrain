...
namespace noise
{
	utils::BuildPlanetResult ComplexPlanetBuilder::BuildPlanetNoises(double southCoord, double northCoord, double westCoord, double eastCoord, int width, int height)
	{
		BasicLogging::Write("Starting ComplexPlanet.BuildPlanetNoises: South(" + std::to_string(static_cast<long double>(southCoord)) + ")/north(" + std::to_string(static_cast<long double>(northCoord)) + ")/west(" + std::to_string(static_cast<long double>(westCoord)) + ")/east(" + std::to_string(static_cast<long double>(eastCoord)) + ")/width(" + std::to_string(static_cast<long double>(width)) + ")/height(" + std::to_string(static_cast<long double>(height)) + ")");
	  ...
	  double SOUTH_COORD = southCoord;//-90;

	  // Northernmost coordinate of elevation grid.
	  double NORTH_COORD = northCoord;//90;

	  // Westernmost coordinate of elevation grid.
	  double WEST_COORD = westCoord;//-180;

	  // Easternmost coordinate of elevation grid.
	  double EAST_COORD = eastCoord;//180;

	  // Width of elevation grid, in points.
	  int GRID_WIDTH = width;//4096;

	  // Height of elevation grid, in points.
	  int GRID_HEIGHT = height;//2048;

	  // Load configuration from file
	  BuiltConfiguration builtConfiguration;
	  LoadBuiltConfiguration(BASE_DIRECTORY + CONFIGURATION_FILE, builtConfiguration);

	  // Circumference of the planet, in meters. 
	  // NOTE: This value is not use at all for the calculation of the terrain
	  const double PLANET_CIRCUMFERENCE = 44236800.0;    

	  // Applied to the variables that determine the detail of the terrain produce a quality equilibrated terrain
	  double DETAIL_PERCENTAGE = 0.1;
	  if (builtConfiguration.DETAIL_LEVEL != 0){
		  if (builtConfiguration.DETAIL_LEVEL == 1){
			  DETAIL_PERCENTAGE = 0.5;
		  }
		  else if (builtConfiguration.DETAIL_LEVEL == 2){
			  DETAIL_PERCENTAGE = 1;
		  }
	  }  
	  ...
	  assert(buildConfiguration.CONTINENT_FREQUENCY >= 1.0);
	  assert(buildConfiguration.CONTINENT_LACUNARITY >= 1.0);
	  assert(buildConfiguration.CONTINENT_LACUNARITY <= 4.0);
	  assert(buildConfiguration.MOUNTAIN_LACUNARITY >= 1.0);
	  assert(buildConfiguration.MOUNTAIN_LACUNARITY <= 4.0);
	  assert(buildConfiguration.HILLS_LACUNARITY >= 1.0);
	  assert(buildConfiguration.HILLS_LACUNARITY <= 4.0);
	  assert(buildConfiguration.PLAINS_LACUNARITY >= 1.0);
	  assert(buildConfiguration.PLAINS_LACUNARITY <= 4.0);
	  assert(buildConfiguration.BADLANDS_LACUNARITY >= 1.0);
	  assert(buildConfiguration.BADLANDS_LACUNARITY <= 4.0);
	  assert(SEA_LEVEL >= -1.0);
	  assert(SEA_LEVEL <= 1.0);
	  assert(SHELF_LEVEL >= -1.0);
	  assert(SHELF_LEVEL <= 1.0);
	  assert(SEA_LEVEL > SHELF_LEVEL);
	  assert(buildConfiguration.MOUNTAINS_AMOUNT >= 0.0);
	  assert(buildConfiguration.MOUNTAINS_AMOUNT <= 1.0);
	  assert(buildConfiguration.HILLS_AMOUNT >= 0.0);
	  assert(buildConfiguration.HILLS_AMOUNT <= 1.0);
	  assert(buildConfiguration.BADLANDS_AMOUNT >= 0.0);
	  assert(buildConfiguration.BADLANDS_AMOUNT <= 1.0);
	  assert(buildConfiguration.MOUNTAINS_AMOUNT <= HILLS_AMOUNT);
	  assert(buildConfiguration.RIVER_DEPTH >= 0.0);
	  ...
	  baseContinentDef_pe0.SetSeed(builtConfiguration.CUR_SEED + 0);
	  baseContinentDef_pe0.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY);
	  baseContinentDef_pe0.SetPersistence (0.5);
	  baseContinentDef_pe0.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
	  baseContinentDef_pe0.SetOctaveCount(14);
	  baseContinentDef_pe0.SetNoiseQuality (QUALITY_STD);

	  // Don't really understand the reason of doing this, creating my own one.
	  ...
	  // 2: [Continent-with-ranges module]: Next, a curve module modifies the
	  //    output value from the continent module. 
	  //    This defines the positions of the mountain ranges.
	  module::Curve baseContinentDef_cu;
	  baseContinentDef_cu.SetSourceModule(0, baseContinentDef_pe0);
	  baseContinentDef_cu.AddControlPoint(-2.0000 + SEA_LEVEL, -1.625 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(-1.0000 + SEA_LEVEL, -1.375 + SEA_LEVEL);
	  ...
	  baseContinentDef_cu.AddControlPoint(0.0625 + SEA_LEVEL, 0.125 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(0.1250 + SEA_LEVEL, 0.250 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(0.2500 + SEA_LEVEL, 0.500 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(0.5000 + SEA_LEVEL, 0.750 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(0.7500 + SEA_LEVEL, 1.000 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(1.0000 + SEA_LEVEL, 1.500 + SEA_LEVEL);
	  baseContinentDef_cu.AddControlPoint(2.0000 + SEA_LEVEL, 2.000 + SEA_LEVEL);
	  ...
	  baseContinentDef_pe1.SetSeed(builtConfiguration.CUR_SEED + 1);
	  baseContinentDef_pe1.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 4.34375);
	  baseContinentDef_pe1.SetPersistence(builtConfiguration.DETAIL_LEVEL == 0 ? 0.250 : 0.5); // Low detail level smooth transitions
	  baseContinentDef_pe1.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
	  ...
	  continentDef_tu0.SetSeed(builtConfiguration.CUR_SEED + 10);
	  continentDef_tu0.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 15.25 * DETAIL_PERCENTAGE);
	  continentDef_tu0.SetPower(builtConfiguration.CONTINENT_FREQUENCY / 113.75);
	  ...
	  continentDef_tu1.SetSeed(builtConfiguration.CUR_SEED + 11);
	  continentDef_tu1.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 47.25 * DETAIL_PERCENTAGE);
	  continentDef_tu1.SetPower(builtConfiguration.CONTINENT_FREQUENCY / 433.75);
	  ...
	  continentDef_tu2.SetSeed(builtConfiguration.CUR_SEED + 12);
	  continentDef_tu2.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 95.25 * DETAIL_PERCENTAGE);
	  continentDef_tu2.SetPower(builtConfiguration.CONTINENT_FREQUENCY / 1019.75);
	  ...
	  continentDef_se.SetEdgeFalloff(builtConfiguration.DETAIL_LEVEL != 0 ? 0.0625 : 0.2); //smaller detail needs longer transitions
	  ...
	  PrepareHighDetailedNoise(builtConfiguration, continentDef, terrainTypeDef, scaledMountainousTerrain, scaledHillyTerrain, scaledBadlandsTerrain, scaledPlainsTerrain);
	  ...
	  continentalShelf_rm.SetSeed(builtConfiguration.CUR_SEED + 130);
	  continentalShelf_rm.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 4.375);
	  continentalShelf_rm.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
	  ...
	  baseContinentElev_sb.SetScale(builtConfiguration.CONTINENT_HEIGHT_SCALE);
	  ...
	  ApplyHighDetailedNoise(builtConfiguration, continentDef, baseContinentElev, scaledPlainsTerrain, scaledHillyTerrain, terrainTypeDef, scaledMountainousTerrain, scaledBadlandsTerrain, continentsWithBadlands);

	  module::Cache riverPositions;

	  // We need to define it here and pass the reference because otherwise we lost the scope of the created object inside the method
	  ...
	  module::River riverPositions_uncached;

	  // river calculation, output must go into riverPositions cache module
	  //PrepareRiverNoiseWorms(buildConfiguration, riverPositions, riverPositions_rm0, riverPositions_cu0, riverPositions_rm1, riverPositions_cu1, riverPositions_mi, riverPositions_tu);
	  PrepareRiverNoiseAlgorithm(builtConfiguration, riverPositions, baseContinentElev, riverPositions_uncached);
	  ...
	  ApplyRiverNoise(builtConfiguration, riverPositions, continentsWithRivers, continentsWithBadlands, baseContinentElev, continentsWithRivers_sb, continentsWithRivers_ad, continentsWithRivers_se);
	...
	//  finalPlanet.SetSourceModule (0, finalPlanet_sb);
	  ...
	  utils::NoiseMap elevGridUnScaled;  
	  ...
	  // group.
	  planet.SetSourceModule(unscaledFinalPlanet);
	  planet.SetDestNoiseMap (elevGridUnScaled);
	  planet.Build ();  
	  ...
	  utils::BuildPlanetResult result;

	  result.elevationGridUnScaledTerrain = elevGridUnScaled;

	  BuildElevGridRivers(builtConfiguration, result, riverPositions, SOUTH_COORD, NORTH_COORD, WEST_COORD, EAST_COORD, GRID_WIDTH, GRID_HEIGHT);

	  BasicLogging::Write("Finished ComplexPlanet.BuildPlanetNoises");

	  return result;
	}

	void ComplexPlanetBuilder::PrepareHighDetailedNoise(BuiltConfiguration &builtConfiguration, module::Cache &continentDef, module::Cache &terrainTypeDef, module::Cache &scaledMountainousTerrain, module::Cache &scaledHillyTerrain, module::Cache &scaledBadlandsTerrain, module::Cache &scaledPlainsTerrain){
		// LOW details does not add mountain, hill, plains or badlands terrains
		if (builtConfiguration.DETAIL_LEVEL != 0){
			...
			terrainTypeDef_tu.SetSeed(builtConfiguration.CUR_SEED + 20);
			terrainTypeDef_tu.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 18.125);
			terrainTypeDef_tu.SetPower(builtConfiguration.CONTINENT_FREQUENCY / 20.59375 * TERRAIN_OFFSET);
			...
			mountainBaseDef_rm0.SetSeed(builtConfiguration.CUR_SEED + 30);
			mountainBaseDef_rm0.SetFrequency(1723.0);
			mountainBaseDef_rm0.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			mountainBaseDef_rm1.SetSeed(builtConfiguration.CUR_SEED + 31);
			mountainBaseDef_rm1.SetFrequency(367.0);
			mountainBaseDef_rm1.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			mountainBaseDef_tu0.SetSeed(builtConfiguration.CUR_SEED + 32);
			...
			mountainBaseDef_tu1.SetSeed(builtConfiguration.CUR_SEED + 33);
			...
			mountainousHigh_rm0.SetSeed(builtConfiguration.CUR_SEED + 40);
			mountainousHigh_rm0.SetFrequency(2371.0);
			mountainousHigh_rm0.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			mountainousHigh_rm1.SetSeed(builtConfiguration.CUR_SEED + 41);
			mountainousHigh_rm1.SetFrequency(2341.0);
			mountainousHigh_rm1.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			mountainousHigh_tu.SetSeed(builtConfiguration.CUR_SEED + 42);
			...
			mountainousLow_rm0.SetSeed(builtConfiguration.CUR_SEED + 50);
			mountainousLow_rm0.SetFrequency(1381.0);
			mountainousLow_rm0.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			mountainousLow_rm1.SetSeed(builtConfiguration.CUR_SEED + 51);
			mountainousLow_rm1.SetFrequency(1427.0);
			mountainousLow_rm1.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			hillyTerrain_bi.SetSeed(builtConfiguration.CUR_SEED + 60);
			hillyTerrain_bi.SetFrequency(1663.0);
			hillyTerrain_bi.SetPersistence(0.5);
			hillyTerrain_bi.SetLacunarity(builtConfiguration.HILLS_LACUNARITY);
			...
			hillyTerrain_rm.SetSeed(builtConfiguration.CUR_SEED + 61);
			hillyTerrain_rm.SetFrequency(367.5);
			hillyTerrain_rm.SetLacunarity(builtConfiguration.HILLS_LACUNARITY);
			...
			hillyTerrain_tu0.SetSeed(builtConfiguration.CUR_SEED + 62);
			...
			hillyTerrain_tu1.SetSeed(builtConfiguration.CUR_SEED + 63);
			...
			plainsTerrain_bi0.SetSeed(builtConfiguration.CUR_SEED + 70);
			plainsTerrain_bi0.SetFrequency(1097.5);
			plainsTerrain_bi0.SetPersistence(0.5);
			plainsTerrain_bi0.SetLacunarity(builtConfiguration.PLAINS_LACUNARITY);
			...
			plainsTerrain_bi1.SetSeed(builtConfiguration.CUR_SEED + 71);
			plainsTerrain_bi1.SetFrequency(1319.5);
			plainsTerrain_bi1.SetPersistence(0.5);
			plainsTerrain_bi1.SetLacunarity(builtConfiguration.PLAINS_LACUNARITY);
			...
			badlandsSand_rm.SetSeed(builtConfiguration.CUR_SEED + 80);
			badlandsSand_rm.SetFrequency(6163.5);
			badlandsSand_rm.SetLacunarity(builtConfiguration.BADLANDS_LACUNARITY);
			...
			badlandsSand_vo.SetSeed(builtConfiguration.CUR_SEED + 81);
			...
			badlandsCliffs_pe.SetSeed(builtConfiguration.CUR_SEED + 90);
			badlandsCliffs_pe.SetFrequency(builtConfiguration.CONTINENT_FREQUENCY * 839.0);
			badlandsCliffs_pe.SetPersistence(0.5);
			badlandsCliffs_pe.SetLacunarity(builtConfiguration.BADLANDS_LACUNARITY);
			...
			badlandsCliffs_tu0.SetSeed(builtConfiguration.CUR_SEED + 91);
			...
			badlandsCliffs_tu1.SetSeed(builtConfiguration.CUR_SEED + 92);
			...
			scaledMountainousTerrain_pe.SetSeed(builtConfiguration.CUR_SEED + 110);
			scaledMountainousTerrain_pe.SetFrequency(14.5);
			scaledMountainousTerrain_pe.SetPersistence(0.5);
			scaledMountainousTerrain_pe.SetLacunarity(builtConfiguration.MOUNTAIN_LACUNARITY);
			...
			scaledHillyTerrain_pe.SetSeed(builtConfiguration.CUR_SEED + 120);
			scaledHillyTerrain_pe.SetFrequency(13.5);
			scaledHillyTerrain_pe.SetPersistence(0.5);
			scaledHillyTerrain_pe.SetLacunarity(builtConfiguration.HILLS_LACUNARITY);
	...
	void ComplexPlanetBuilder::ApplyHighDetailedNoise(BuiltConfiguration &builtConfiguration, module::Cache &continentDef, module::Cache &baseContinentElev, module::Cache &scaledPlainsTerrain, module::Cache &scaledHillyTerrain, module::Cache &terrainTypeDef, module::Cache &scaledMountainousTerrain, module::Cache &scaledBadlandsTerrain, module::Cache &continentsWithBadlands){
		// LOW details does not add mountain, hill, plains or badlands terrains
		if (builtConfiguration.DETAIL_LEVEL != 0){
			...
			continentsWithHills_se.SetBounds(1.0 - builtConfiguration.HILLS_AMOUNT, 1001.0 - builtConfiguration.HILLS_AMOUNT);
			...
			continentsWithMountains_cu.AddControlPoint(1.0 - builtConfiguration.MOUNTAINS_AMOUNT, 0.0625);
			...
			continentsWithMountains_se.SetBounds(1.0 - builtConfiguration.MOUNTAINS_AMOUNT, 1001.0 - builtConfiguration.MOUNTAINS_AMOUNT);
			...
			continentsWithBadlands_pe.SetSeed(builtConfiguration.CUR_SEED + 140);
			continentsWithBadlands_pe.SetFrequency(16.5);
			continentsWithBadlands_pe.SetPersistence(0.5);
			continentsWithBadlands_pe.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
			...
			continentsWithBadlands_se.SetBounds(1.0 - builtConfiguration.BADLANDS_AMOUNT, 1001.0 - builtConfiguration.BADLANDS_AMOUNT);
	...
	void ComplexPlanetBuilder::PrepareRiverNoiseWorms(BuiltConfiguration &builtConfiguration, module::Cache &riverPositions, module::RidgedMulti &riverPositions_rm0, module::Curve &riverPositions_cu0, module::RidgedMulti &riverPositions_rm1, module::Curve &riverPositions_cu1, module::Min &riverPositions_mi, module::Turbulence &riverPositions_tu){
		if (builtConfiguration.GENERATE_RIVERS) {
			////////////////////////////////////////////////////////////////////////////
			...
			//module::RidgedMulti riverPositions_rm0;
			riverPositions_rm0.SetSeed(builtConfiguration.CUR_SEED + 100);
			riverPositions_rm0.SetFrequency(18.75);
			riverPositions_rm0.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
			...
			//module::RidgedMulti riverPositions_rm1;
			riverPositions_rm1.SetSeed(builtConfiguration.CUR_SEED + 101);
			riverPositions_rm1.SetFrequency(43.25);
			riverPositions_rm1.SetLacunarity(builtConfiguration.CONTINENT_LACUNARITY);
			...
			riverPositions_cu1.AddControlPoint(-2.000, 2.000);
			riverPositions_cu1.AddControlPoint(-1.000, 1.000);
			riverPositions_cu1.AddControlPoint(-0.125, 0.875);
			riverPositions_cu1.AddControlPoint(0.000, -1.000);
			riverPositions_cu1.AddControlPoint(1.000, -1.500);
			riverPositions_cu1.AddControlPoint(2.000, -2.000);
			...
			riverPositions_tu.SetSeed(builtConfiguration.CUR_SEED + 102);
	...
	void ComplexPlanetBuilder::PrepareRiverNoiseAlgorithm(BuiltConfiguration &builtConfiguration, module::Cache &riverPositions, module::Cache &baseContinentElev, module::River &riverPositions_uncached){
		if (builtConfiguration.GENERATE_RIVERS) {
			////////////////////////////////////////////////////////////////////////////
			// Module group: river positions
			////////////////////////////////////////////////////////////////////////////

			riverPositions_uncached.SetSourceModule(0, baseContinentElev);
			riverPositions.SetSourceModule(0, riverPositions_uncached);
		}
	}

	void ComplexPlanetBuilder::ApplyRiverNoise(BuiltConfiguration &builtConfiguration, module::Cache &riverPositions, module::Cache &continentsWithRivers, module::Cache &continentsWithBadlands, module::Cache &baseContinentElev, module::ScaleBias &continentsWithRivers_sb, module::Add &continentsWithRivers_ad, module::Select &continentsWithRivers_se){
		if (builtConfiguration.GENERATE_RIVERS) {
			////////////////////////////////////////////////////////////////////////////
			...
			continentsWithRivers_sb.SetScale(builtConfiguration.RIVER_DEPTH / 2.0);
			continentsWithRivers_sb.SetBias(-builtConfiguration.RIVER_DEPTH / 2.0);
			...
			if (builtConfiguration.DETAIL_LEVEL != 0) {
				...
				continentsWithRivers_ad.SetSourceModule(0, baseContinentElev);
			}
			...
			if (builtConfiguration.DETAIL_LEVEL != 0) {
				...
				continentsWithRivers_se.SetSourceModule(0, baseContinentElev);
				continentsWithRivers_se.SetSourceModule(1, continentsWithRivers_ad);
				continentsWithRivers_se.SetControlModule(baseContinentElev);
			}			

			continentsWithRivers_se.SetBounds(SEA_LEVEL, builtConfiguration.CONTINENT_HEIGHT_SCALE + SEA_LEVEL);
			continentsWithRivers_se.SetEdgeFalloff((builtConfiguration.CONTINENT_HEIGHT_SCALE * 10) - SEA_LEVEL);
			...
			continentsWithRivers.SetSourceModule(0, baseContinentElev);
		}		
	}

	void ComplexPlanetBuilder::BuildElevGridRivers(BuiltConfiguration &builtConfiguration, utils::BuildPlanetResult &result, module::Cache &riverPositions, double southCoord, double northCoord, double westCoord, double eastCoord, int width, int height){
		if (builtConfiguration.GENERATE_RIVERS){
			utils::NoiseMapBuilderSphere planetRivers;
			//utils::NoiseMap elevGridDepthRivers;
			//utils::NoiseMap elevGridShallowRivers;
			utils::NoiseMap elevGridRivers;

			planetRivers.SetBounds(southCoord, northCoord, westCoord, eastCoord);
			planetRivers.SetDestSize(width, height);
			/*planetRivers.SetSourceModule(riverPositions_cu0);
			planetRivers.SetDestNoiseMap(elevGridDepthRivers);
			planetRivers.Build();
			planetRivers.SetSourceModule(riverPositions_cu1);
			planetRivers.SetDestNoiseMap(elevGridShallowRivers);
			planetRivers.Build();*/
			planetRivers.SetSourceModule(riverPositions);
			planetRivers.SetDestNoiseMap(elevGridRivers);
			planetRivers.Build();

			//result.elevationGridDepthRivers = elevGridDepthRivers;
			//result.elevationGridShallowRivers = elevGridShallowRivers;
			result.elevationGridRivers = elevGridRivers;
			result.hasRivers = true;
		}
		else {
			result.hasRivers = false;
		}
	}

	void ComplexPlanetBuilder::LoadBuiltConfiguration(const std::string &configFile, BuiltConfiguration &builtConfiguration)
	{
		BasicLogging::Write("ComplexPlat.LoadBuiltConfiguration: Reading built configuration from " + configFile + " file");
		...
		while (std::getline(fin, line)) {
			if (line.find("//") == std::string::npos){
				...
				if (line.find("CUR_SEED") != std::string::npos) {
					//std::cout << "CUR_SEED" << sin.str() << std::endl;
					sin >> builtConfiguration.CUR_SEED;
				}
				else if (line.find("CONTINENT_FREQUENCY") != std::string::npos) {
					sin >> builtConfiguration.CONTINENT_FREQUENCY;
				}
				else if (line.find("CONTINENT_LACUNARITY") != std::string::npos) {
					sin >> builtConfiguration.CONTINENT_LACUNARITY;
				}
				else if (line.find("MOUNTAIN_LACUNARITY") != std::string::npos) {
					sin >> builtConfiguration.MOUNTAIN_LACUNARITY;
				}
				else if (line.find("HILLS_LACUNARITY") != std::string::npos) {
					sin >> builtConfiguration.HILLS_LACUNARITY;
				}
				else if (line.find("PLAINS_LACUNARITY") != std::string::npos) {
					sin >> builtConfiguration.PLAINS_LACUNARITY;
				}
				else if (line.find("BADLANDS_LACUNARITY") != std::string::npos) {
					sin >> builtConfiguration.BADLANDS_LACUNARITY;
				}
				else if (line.find("MOUNTAINS_AMOUNT") != std::string::npos) {
					sin >> builtConfiguration.MOUNTAINS_AMOUNT;
				}
				else if (line.find("HILLS_AMOUNT") != std::string::npos) {
					sin >> builtConfiguration.HILLS_AMOUNT;
				}
				else if (line.find("BADLANDS_AMOUNT") != std::string::npos) {
					sin >> builtConfiguration.BADLANDS_AMOUNT;
				}
				else if (line.find("RIVER_DEPTH") != std::string::npos) {
					sin >> builtConfiguration.RIVER_DEPTH;
				}
				else if (line.find("GENERATE_RIVERS") != std::string::npos) {
					sin >> builtConfiguration.GENERATE_RIVERS;
				}
				else if (line.find("DETAIL_LEVEL") != std::string::npos) {
					sin >> builtConfiguration.DETAIL_LEVEL;
				}
				else if (line.find("CONTINENT_HEIGHT_SCALE") != std::string::npos) {
					sin >> builtConfiguration.CONTINENT_HEIGHT_SCALE;
				}
			}
			sin.clear();
		}
